...
function ModificationByField(props) {
    ...
    useEffect(() => {
        ...
        let endMonth = endDate.getMonth() + 1 < 10
            ? `0${endDate.getMonth() + 1}`
            : endDate.getMonth() + 1;
        let startMonth = startDate.getMonth() + 1 < 10
    ...
    const [UiObjs,
        setUiObjs] = useState([]); //UiObject from the server
    const [fieldNameOptions,
        setFieldNameOptions] = useState([]); //FieldName options for filtering
    const [valueOptions,
        setValueOptions] = useState([]); //Values of certain FieldName options for filtering
    const [startDate,
        setStartDate] = useState("");
    const [endDate,
        setEndDate] = useState("");
    const [qaRepresentativeOptions,
        setQaRepresentativeOptions] = useState([]); //Qa Representative options for filtering
    const labelOptions = [
        { //Labels for Displaying the Chartvalue: "daily", label: "daily"
        }, {
            value: "weekly",
            label: "weekly"
        }, {
    ...
    // handleSaveFilter Filters Changes Handlers for each Filter we have a handler
    // to update serverFilters that we send to server to receive data according to
    // our picks Label
    ...
    // Filters Section Modification By Field save filters Variables Filters Variable
    // to save the filters data
    let savedFilters = {
        pageName: 'ModificationByField',
        filters: [
            {
                filter: 'fieldName',
                values: []
    ...
    let [selectFiltersOptions,
        setSelectFiltersOptions] = useState([]);

    //select filter option for the filter
    let [savedFiltersArray,
        setSavedFiltersArray] = useState([]);

    //a Variable for the filter name
    let filterName = '';

    // a Variable for the selected filter option let selectFilterCurrentOption = '';
    let [selectFilterCurrentOption,
        setSelectFilterCurrentOption] = useState("");
    ...
    const handleSaveFilter = (e => {
        savedFilters
            .filters[0]
            .values
            .push(serverFilters.fieldName);
        savedFilters
            .filters[1]
            .values
            .push(serverFilters.values);
        savedFilters
            .filters[2]
            .values
            .push(serverFilters.qaRepresentative);
        savedFilters
            .filters[3]
            .values
            .push(serverFilters.label[0]);
    ...
    const handleSelectFilter = (change => {
        ...
        FieldNameInput.current.state.value = {
            label: savedFiltersArray[index].filters[0].values
        }
        valueInput.current.state.value = {
            label: savedFiltersArray[index].filters[1].values
        }
        qaInput.current.state.value = {
            label: savedFiltersArray[index].filters[2].values
        }
        periodInput.current.state.value = {
            label: savedFiltersArray[index].filters[3].values
        }

        if (savedFiltersArray[index].filters[0].values != null) {
            savedFiltersArray[index]
                .filters[0]
                .values
                .map((item) => {
                    serverFilters
        ...
        <div className='ModificationByField__Wrapper'>
            ...
            <div className="ModificationByField__Filters__wrapper">
                ...
                <div className="ModificationByField__Filters__fields">
                    ...
                    <div className="ModificationByField__Filters__Header"></div>
