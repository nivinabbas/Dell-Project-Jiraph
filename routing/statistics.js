...
router.post("/getStatistics", async function (req, res) {
  console.log("aa");
  let { startDate, endDate } = req.body; // 4 , 7,10
    ...
    },
    {
      $project: {
        DifferenceInDays: {
          $round: {
            $divide: [
              { $subtract: ["$taskItem.updatedTime", "$taskItem.createdTime"] },
              86400000,
            ],
          },
        },
  ...
  completedTasks22.forEach((element) => {
    objIndex = arrayToClint.findIndex(
      (obj) => obj.date == element.DifferenceInDays
    );
    if (objIndex >= 0) {
...

    {
      $project: {
        DifferenceInDays: {
          $round: {
            $divide: [
              { $subtract: [new Date(), "$taskItem.createdTime"] },
              86400000,
            ],
          },
        },
  ...
  noTcompletedTasks.forEach((element) => {
    objIndex = arrayToClintNotCompleted.findIndex(
      (obj) => obj.day == element.DifferenceInDays
    );
    if (objIndex >= 0) {
      arrayToClintNotCompleted[objIndex].cunt =
        arrayToClintNotCompleted[objIndex].cunt + 1;
    } else {
