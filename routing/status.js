...
router.post("/openTasksWithFilter", [auth, qaManager], async function (
  req,
  res
) {
...
router.post("/stackedChart", [auth, qaManager], async function (req, res) {
  ...
  if (formatLabel != "weekly") {
    ...
    function getDateRangeOfWeek(weekNo) {
      ...
      if (
        eval(d1.getMonth() + 1).toString().length == 1 &&
        d1.getDate().toString().length == 1
      ) {
        rangeIsFrom =
          d1.getFullYear() +
          "-" +
        ...
        eval(d1.getMonth() + 1).toString().length == 2 &&
        d1.getDate().toString().length == 1
      ) {
        rangeIsFrom =
          d1.getFullYear() +
    ...
    resultWeek[resultWeek.length - 1].date =
      LastDate[0] + " " + LastDate[1] + " " + NewLastDay;
...
router.get("/modificationTypeOptions", [auth, qaManager], async function (
  req,
  res
) {
  let Data = [
...
router.get("/modificationFieldOptions", [auth, qaManager], async function (
  req,
  res
) {
  let Data = [];
...
router.post("/segmentData", async function (req, res) {
  let { date, status } = req.body; // 4 , 7,10 ,weekly
  ...
  console.log(
    "End segmentData End segmentData End segmentData End segmentData"
  );
  res.send({
    success: true,
    error: null,
